...
public class ConfigItemDTO extends ResponseDTO {
	...
	@ArraySchema(minItems = 0, maxItems = 10000, schema = @Schema(implementation = UserEty.class))
    List<UserEty> configurationItems;

    @Size(min = 0, max = 10000)
    @Schema(description = "Numero di elementi di configurazione")
    Integer size;

    public ConfigItemDTO(final LogTraceInfoDTO traceInfo, final List<UserEty> inConfigurationItems, final Integer inSize) {
        super(traceInfo);
